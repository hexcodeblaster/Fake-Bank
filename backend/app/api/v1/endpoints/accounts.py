...
async def create_account(account: AccountCreate, db: Session = Depends(get_db), account_service: AccountService = Depends(get_account_service)) -> AccountResponse:
...
async def withdraw(account_id: int, payload: AmountRequest, db: Session = Depends(get_db), account_service: AccountService = Depends(get_account_service)) -> AccountResponse:
...
async def deposit(account_id: int, payload:AmountRequest, db: Session = Depends(get_db), account_service: AccountService = Depends(get_account_service)) -> AccountResponse:
...
async def check_balance(account_id: int, db: Session = Depends(get_db), account_service: AccountService = Depends(get_account_service)) -> Decimal:
